...
import { isWindowsDevice } from '@/lib/device';

import './Dither.css';

// ── Original Shaders & Post-processing for macOS ─────────────────────
const waveVertexShaderOriginal = /* glsl */ `
...
const waveFragmentShaderOriginal = /* glsl */ `
precision highp float;
...
void main() {
  ...
  if (enableMouseInteraction == 1 && mousePos.x > -5000.0) {
    vec2 mouseNDC = (mousePos / resolution - 0.5) * vec2(1.0, -1.0);
    mouseNDC.x *= resolution.x / resolution.y;
    float dist = length(uv - mouseNDC);
    float effect = 1.0 - smoothstep(0.0, mouseRadius, dist);
    f = max(0.0, f - 0.85 * effect);
  }
...
const ditherFragmentShaderOriginal = /* glsl */ `
precision highp float;
...
class RetroEffectImpl extends Effect {
  constructor() {
    ...
    super('RetroEffect', ditherFragmentShaderOriginal, { uniforms });
  }
...
function DitheredWavesOriginal({
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (inBounds) {
        mouseTargetRef.current.set(e.clientX - rect.left, e.clientY - rect.top);
      } else {
        mouseTargetRef.current.set(-9999, -9999);
      }
    };
  ...
  useFrame(({ clock }) => {
    ...
    if (enableMouseInteraction) {
      if (mouseTargetRef.current.x > -5000) {
        if (mouseCurrentRef.current.x < -5000) {
          mouseCurrentRef.current.copy(mouseTargetRef.current);
        } else {
          mouseCurrentRef.current.lerp(mouseTargetRef.current, 0.15);
        }
      } else {
        mouseCurrentRef.current.set(-9999, -9999);
      }
      u.mousePos.value.copy(mouseCurrentRef.current);
    }
  });
  ...
  return (
    <>
      <mesh scale={[viewport.width, viewport.height, 1]}>
        ...
        <shaderMaterial
          vertexShader={waveVertexShaderOriginal}
          fragmentShader={waveFragmentShaderOriginal}
...
// ── Single-pass Shader for Windows (Zero-lag) ────────────────────────
const waveVertexShaderWin = /* glsl */ `
precision highp float;
varying vec2 vUv;
void main() {
  vUv = uv;
...
const waveFragmentShaderWin = /* glsl */ `
precision highp float;
uniform vec2 resolution;
uniform float time;
uniform float waveSpeed;
...
function DitheredWavesWin({
  ...
  return (
    <mesh scale={[viewport.width, viewport.height, 1]}>
      <planeGeometry args={[1, 1]} />
      <shaderMaterial
        vertexShader={waveVertexShaderWin}
        fragmentShader={waveFragmentShaderWin}
        uniforms={waveUniformsRef.current}
      />
    </mesh>
  );
